...
class ExtendedBadgeEnrollmentActor @Inject()(@Named("course-batch-notification-actor") courseBatchNotificationActorRef: ActorRef)(implicit val cacheUtil: RedisCacheUtil)
            ...
            try {
              val formats = List(
                ...
                "yyyy-MM-dd'T'HH:mm:ss'Z'" )
                  formats.view.flatMap { pattern => try { val sdf = new java.text.SimpleDateFormat(pattern)
                  sdf.setTimeZone(java.util.TimeZone.getTimeZone("UTC"))
                  Some(sdf.parse(dateStr).getTime) } catch { case _: Exception => None } }.headOption.getOrElse(0L) }
            catch {
              case _: Exception => 0L
            }
  ...
  private def fetchBadgeCoursesFromSearchBatch(
    ...
    try {
      ...
      logger.info(request.getRequestContext,
        s"fetchBadgeCoursesFromSearchBatch :: Calling search API for ${courseIds.size()} course IDs")
