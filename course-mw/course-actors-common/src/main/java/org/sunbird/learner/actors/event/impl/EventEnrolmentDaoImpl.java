...
public class EventEnrolmentDaoImpl implements EventEnrolmentDao {
    ...
    if (!((List<Map<String, Object>>) res.get(JsonKey.RESPONSE)).isEmpty()) {
      ...
      for (Map<String, Object> enrollment : userEnrollmentList) {
        ...
        Map<String, Object> contentDetails =
            getEventDetails(
                request.getRequestContext(), (String) enrollment.get(JsonKey.CONTENT_ID));
        List<Map<String, Object>> batchDetails =
            getBatchList(request, contentId, contextId, batchId);
  ...
  private List<Map<String, Object>> getUserEventConsumption(
      Request request, String userId, String contentId, String contextId, String batchId) {
  ...
  public List<Map<String, Object>> getBatchList(
      Request request, String contentId, String contextId, String batchId) {
    logger.info(
        request.getRequestContext(),
        "EventEnrolmentDaoImpl:getBatchList: eventId = " + contentId + " batchId = " + batchId);
  ...
  @Override
  public List<Map<String, Object>> getUserEventEnrollment(
      Request request, String userId, String eventId, String batchId) {
    logger.info(
        request.getRequestContext(),
        "EventEnrolmentDaoImpl:getUserEventEnrollment: UserId = "
            + userId
            + " eventId = "
            + eventId
            + " batchId = "
  ...
  private Map<String, Object> getEventDetails(RequestContext requestContext, String eventId) {
    logger.info(
        requestContext, "EventEnrolmentDaoImpl:getEventDetails: eventId: " + eventId, null, null);
    Map<String, Object> response = new HashMap<>();
    try {
      String key = getCacheKey(eventId);
      int ttl =
          Integer.parseInt(PropertiesCache.getInstance().getProperty(JsonKey.EVENT_REDIS_TTL));
      String cacheResponse = redisCacheUtil.get(key, null, ttl);
      ObjectMapper mapper = new ObjectMapper();
      if (cacheResponse != null
          && !cacheResponse.trim().isEmpty()
          && !cacheResponse.trim().equals("{}")) {
        logger.info(
            requestContext,
            "EventEnrolmentDaoImpl:getContentDetails: Data reading from cache ",
            null,
            null);
        ...
        logger.debug(
            requestContext,
            "EventEnrolmentDaoImpl:getContentDetails: courseId: " + eventId,
            null,
            ekStepContent);
  ...
  public Map<String, Object> getUserDetails(String userId, RequestContext requestContext) {
    try {
      Response response =
          cassandraOperation.getUserRecordFromDB(
              JsonKey.KEYSPACE_SUNBIRD, JsonKey.TABLE_USER, userId, requestContext);
  ...
  public List<Map<String, Object>> getEventEnrolmentList(Request request, String userId) {
    ...
    int limit = request.get(JsonKey.LIMIT) != null ? (int) request.get(JsonKey.LIMIT) : -1;
    if (res != null
        && res.containsKey(JsonKey.RESPONSE)
        && res.get(JsonKey.RESPONSE) instanceof List
        && !((List<Map<String, Object>>) res.get(JsonKey.RESPONSE)).isEmpty()) {
      userEnrollmentList = ((List<Map<String, Object>>) res.get(JsonKey.RESPONSE));
      if (CollectionUtils.isNotEmpty(userEnrollmentList)) {
        if (StringUtils.isNotEmpty(status) && statusMap.get(status) != null) {
          if (statusMap.get(status) == 1) {
  ...
  private void addEventDetailsToEnrollment(
      ...
      String actualEventType,
      String requestedEventType,
      boolean calendarEventEnabled) {
    if (calendarEventEnabled || requestedEventType.equalsIgnoreCase(actualEventType)) {
