...
public class EventManagementActor extends BaseActor {
	...
	private void discardEvent(Request request) throws Exception {
		...
		Response response = EventContentUtil.deleteContent(request, "/private/event/v4/discard/{identifier}", pathId,
				pathVal);
		try {
			if (response != null && response.getResponseCode().getResponseCode() == ResponseCode.OK.getResponseCode()) {
				...
				ProjectCommonException.throwClientErrorException(ResponseCode.customServerError,
	...
	private void validateNoEnrollments(Request request) {
		...
		String batchId = EventUtil.formBatchIdForFixedBatchId(identifier, fixedBatchId);
		List<String> participants = userCoursesService.getParticipantsList(batchId, true, request.getRequestContext());
		if (!participants.isEmpty()) {
			ProjectCommonException.throwClientErrorException(ResponseCode.cannotUpdateEventSetHavingEnrollments,
